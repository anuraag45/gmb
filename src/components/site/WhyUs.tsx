        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {WHY_US_CARDS.map((card, idx) => {
            ...
            return (
              <div
                key={card.id}
                className="bg-white rounded-lg border border-slate-200/90 p-7 flex flex-col justify-between hover:shadow-md hover:border-slate-300 transition-all duration-200 group"
              >
                <div>
                  {/* Icon Badge */}
                  <div className="w-11 h-11 rounded-lg bg-[#EBF3FA] text-[#0B2F52] flex items-center justify-center mb-6 group-hover:bg-[#0B2F52] group-hover:text-white transition-colors duration-200">
                    <IconComponent className="w-5 h-5 stroke-[1.8]" />
                  </div>

                  {/* Title */}
                  <h3 className="text-lg font-bold text-slate-900 mb-3 tracking-tight font-sans">
                    {title}
                  </h3>
